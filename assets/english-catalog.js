...
(() => {
  ...
  function quote(card){const p=card.psa10;return money(p?.sourcePriceCents)?`PSA10 ${p.status==='available'?'참고가':p.status==='unverified'?'참고가 · 확인 필요':'추정가'} ${money(p.sourcePriceCents)}`:p?.status==='no-price'?'PSA10 가격 없음':'PSA10 미수집';}
  function setBody(){
    if(!active)return;
  ...
  async function open(code,keep=false){
    ...
    try{
      ...
      if(!data){const r=await fetch(`/data/english-sets/${encodeURIComponent(code)}.json?v=20260908-en1`);if(!r.ok)throw Error();data=await r.json();if(data.language!=='en'||data.code!==code||data.cards?.length!==set.cardCount)throw Error();cache.set(code,data);if(cache.size>8)cache.delete(cache.keys().next().value);}
  ...
  const ready=fetch('/data/english-catalog.json?v=20260908-en1').then(r=>{if(!r.ok)throw Error();return r.json();}).then(data=>{if(data.schemaVersion!==1||!Array.isArray(data.sets))throw Error();catalog=data;return data;});

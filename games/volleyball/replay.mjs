import { Match } from './engine.mjs?v=5';
...
export function verifyReplay(options, replay) {
  if (!replay || replay.version !== RULES_VERSION || !Number.isInteger(replay.ticks) || replay.ticks < 1 || replay.ticks > MAX_TICKS ||
  ...
  const match = new Match(options); match.start(); let cursor = 0, controls = decodeInput(0);

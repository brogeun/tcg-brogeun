...
import {caughtCount,requirementsMet} from './core.mjs';
...
export class Expedition{
 ...
 refreshUnlocks(quiet=false){const g=this.game,e=this.state,count=caughtCount(g.save),caughtRoots=new Set(Object.entries(g.save.dex).filter(([,d])=>d.caught).map(([id])=>this.rootById.get(+id)));
  for(const cap of this.compositions.fieldCapabilities)if(!e.capabilities.includes(cap.id)&&(count>=cap.grant.orCaughtCount||cap.grant.caughtAnyFamilyRootIds.some(id=>caughtRoots.has(id)))){e.capabilities.push(cap.id);if(!quiet){g.notify(t('exp.capability'),worldText(cap.name));g.audio.play('level');}}
  const pending=MILESTONES.filter(n=>count>=n&&!e.claimedMilestones.includes(n));for(const n of pending){e.claimedMilestones.push(n);g.save.inventory['great-ball']=(g.save.inventory['great-ball']||0)+(n>=20?5:2);g.save.inventory['ultra-ball']=(g.save.inventory['ultra-ball']||0)+(n>=50?5:0);g.save.inventory['rare-candy']=(g.save.inventory['rare-candy']||0)+1;g.save.coins+=100+n*8;}
 ...
 captured(id){const e=this.state,g=this.game;e.streak++;this.mark('capture');if(g.save.playTime<600&&g.player&&g.save.collection.length<=4){g.player.hp=Math.min(g.player.maxHp,g.player.hp+Math.ceil(g.player.maxHp*.35));g.syncPlayer();}const research=e.research[g.save.region]??={caught:[],done:false};if(!research.caught.includes(id))research.caught.push(id);if(research.caught.length>=3&&!research.done){research.done=true;e.tokens+=2;g.save.inventory['great-ball']=(g.save.inventory['great-ball']||0)+6;g.notify(t('exp.researchDone'),entity('items','great-ball')+' +6');}if(e.streak%3===0){g.save.inventory['poke-ball']+=5;g.notify(t('exp.streak',{count:e.streak}),t('exp.reward',{balls:5,funds:0}));}this.refreshUnlocks();}
 ...
 discover(l){if(this.state.landmarks.includes(l.id))return;this.state.landmarks.push(l.id);this.state.tokens++;this.mark('landmark');const g=this.game;g.save.inventory['poke-ball']+=4;g.save.coins+=120;g.notify(t('exp.landmark'),worldText(l.name)+' · '+t('exp.reward',{balls:4,funds:120}));g.audio.play('register');g.world.ring(new THREE.Vector3(l.position.x,g.world.height(l.position.x,l.position.z),l.position.z),0xd9e9a6,4,1.2);this.refreshUnlocks();g.persist();}
